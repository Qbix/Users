...
Users.listen = function (options) {
	...
	server.addMethod('Users/intentComplete', function (parsed) {
		if (parsed.token) {
			// the waiting (anonymous) client joined this room when it started
			// the intent; the token is already in its possession
			Users.Socket.emitToTopic('Users/intent/' + parsed.token,
				'Users/intentComplete', { token: parsed.token });
		}
		if (parsed.userId) {
...
/**
 * Link to Users.Socket
 * @property Socket
 * @type Users.Socket
 */
Users.Socket = Q.require('Users/Socket');
