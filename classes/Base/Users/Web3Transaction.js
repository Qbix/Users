/**
 * Autogenerated base class representing web3_transaction rows
 * in the Users database.
 *
 * Don't change this file, since it can be overwritten.
 * Instead, change the Users/Web3Transaction.js file.
 *
 * @module Users
 */
...
/**
 * Base class representing 'Web3Transaction' rows in the 'Users' database
...
Base.table = function (withoutDbName) {
	if (Q.Config.get(['Db', 'connections', 'Users', 'indexes', 'Web3Transaction'], false)) {
		return new Db.Expression((withoutDbName ? '' : '{{dbname}}.')+'{{prefix}}web3_transaction');
	} else {
		var conn = Db.getConnection('Users');
		var prefix = conn.prefix || '';
		var tableName = prefix + 'web3_transaction';
 ...
 * @type string
 */
Base.prototype.className = "Users_Web3Transaction";
 ...
 * Method is called before setting the field and verifies if value is string of length within acceptable limit.
 * Optionally accept numeric value which is converted to string
 * @method beforeSet_transactionId
 * @param {string} value
 * @return {string} The value
 * @throws {Error} An exception is thrown if 'value' is not string or is exceedingly long
 */
Base.prototype.beforeSet_transactionId = function (value) {
		if (value == null) {
			value='';
		}
		if (value instanceof Db.Expression) return value;
		if (typeof value !== "string" && typeof value !== "number")
			throw new Error('Must pass a String to '+this.table()+".transactionId");
		if (typeof value === "string" && value.length > 100)
			throw new Error('Exceedingly long value being assigned to '+this.table()+".transactionId");
		return value;
};

	/**
	 * Returns the maximum string length that can be assigned to the transactionId field
	 * @return {integer}
	 */
Base.prototype.maxSize_transactionId = function () {

		return 100;
};

	/**
	 * Returns schema information for transactionId column
	 * @return {array} [[typeName, displayRange, modifiers, unsigned], isNull, key, default]
	 */
Base.column_transactionId = function () {
...
Base.prototype.beforeSave = function (value) {
	var fields = ['chainId','transactionId'], i;
	if (!this._retrieved) {
