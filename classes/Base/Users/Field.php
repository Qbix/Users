...
/**
 * Autogenerated base class representing field rows
 * in the Users database.
 *
 * Don't change this file, since it can be overwritten.
 * Instead, change the Users_Field.php file.
 *
 * @module Users
 */
/**
 * Base class representing 'Field' rows in the 'Users' database
 * @class Base_Users_Field
 * @extends Db_Row
 *
...
abstract class Base_Users_Field extends Db_Row
{
	...
	{
		if (class_exists('Q_Config') and Q_Config::get('Db', 'connections', 'Users', 'indexes', 'Field', false)) {
			return new Db_Expression(($with_db_name ? '{{dbname}}.' : '').'{{prefix}}'.'field');
		} else {
			$conn = Db::getConnection('Users');
  			$prefix = empty($conn['prefix']) ? '' : $conn['prefix'];
  			$table_name = $prefix . 'field';
  			if (!$with_db_name)
  				return $table_name;
  			$db = Db::connect('Users');
	...
	static function insertManyAndExecute($rows = array(), $options = array())
	{
		self::db()->insertManyAndExecute(
			self::table(), $rows,
			array_merge($options, array('className' => 'Users_Field'))
		);
	}
...
return array (
  ...
  3 => NULL,
	 ...
	 * Method is called before setting the field and verifies if value is string of length within acceptable limit.
	 * Optionally accept numeric value which is converted to string
	 * @method beforeSet_access
	 * @param {string} $value
	 * @return {array} An array of field name and value
	 * @throws {Exception} An exception is thrown if $value is not string or is exceedingly long
	 */
	function beforeSet_access($value)
	{
		if (!isset($value)) {
			return array('access', $value);
		}
		if ($value instanceof Db_Expression
               or $value instanceof Db_Range) {
			return array('access', $value);
		...
		if (!is_string($value) and !is_numeric($value))
			throw new Exception('Must pass a string to '.$this->getTable().".access");
		if (strlen($value) > 255)
			throw new Exception('Exceedingly long value being assigned to '.$this->getTable().".access");
		return array('access', $value);			
	}

	/**
	 * Returns the maximum string length that can be assigned to the access field
	 * @return {integer}
	 */
	function maxSize_access()
	{
	...
	/**
	 * Returns schema information for access column
	 * @return {array} [[typeName, displayRange, modifiers, unsigned], isNull, key, default]
	 */
	static function column_access()
	{
