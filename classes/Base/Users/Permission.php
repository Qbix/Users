 ...
 * Instead, change the Users_Permission.php file.
 *
 * @module Users
 */
/**
 * Base class representing 'Permission' rows in the 'Users' database
 * @class Base_Users_Permission
 * @extends Db_Row
 *
 ...
 */
abstract class Base_Users_Permission extends Db_Row
{
	/**
	 * @property $userId
	 * @type string
	...
	{
		if (class_exists('Q_Config') and Q_Config::get('Db', 'connections', 'Users', 'indexes', 'Permission', false)) {
			return new Db_Expression(($with_db_name ? '{{dbname}}.' : '').'{{prefix}}'.'permission');
		} else {
			$conn = Db::getConnection('Users');
  			$prefix = empty($conn['prefix']) ? '' : $conn['prefix'];
  			$table_name = $prefix . 'permission';
  			if (!$with_db_name)
  				return $table_name;
  			$db = Db::connect('Users');
...
return array (
  ...
  3 => NULL,
	 ...
	 * @throws {Exception} An exception is thrown if $value is not string or is exceedingly long
	 */
	function beforeSet_permission($value)
	{
		if (!isset($value)) {
			$value='';
		}
		...
		if (!is_string($value) and !is_numeric($value))
			throw new Exception('Must pass a string to '.$this->getTable().".permission");
		if (strlen($value) > 255)
			throw new Exception('Exceedingly long value being assigned to '.$this->getTable().".permission");
		return array('permission', $value);			
	}
	 ...
	 * @return {array} [[typeName, displayRange, modifiers, unsigned], isNull, key, default]
	 */
	static function column_permission()
	{
...
return array (
  ...
  3 => NULL,
	...
	function beforeSave($value)
	{
		...
		if (!isset($this->fields["permission"]) and !isset($value["permission"])) {
	...
	{
		$field_names = array('userId', 'label', 'permission', 'extra', 'insertedTime', 'updatedTime');
		$result = $field_names;
