 ...
 * Instead, change the Users_Intent.php file.
 *
 * @module Users
 */
/**
 * Base class representing 'Intent' rows in the 'Users' database
 * @class Base_Users_Intent
 * @extends Db_Row
 *
...
abstract class Base_Users_Intent extends Db_Row
{
	...
	{
		if (class_exists('Q_Config') and Q_Config::get('Db', 'connections', 'Users', 'indexes', 'Intent', false)) {
			return new Db_Expression(($with_db_name ? '{{dbname}}.' : '').'{{prefix}}'.'intent');
		} else {
			$conn = Db::getConnection('Users');
  			$prefix = empty($conn['prefix']) ? '' : $conn['prefix'];
  			$table_name = $prefix . 'intent';
  			if (!$with_db_name)
  				return $table_name;
  			$db = Db::connect('Users');
	...
	static function insertManyAndExecute($rows = array(), $options = array())
	{
		self::db()->insertManyAndExecute(
			self::table(), $rows,
			array_merge($options, array('className' => 'Users_Intent'))
		);
	}
...
return array (
  ...
  3 => NULL,
	 ...
	 * @throws {Exception} An exception is thrown if $value is not string or is exceedingly long
	 */
	function beforeSet_action($value)
	{
		if (!isset($value)) {
			$value='';
		}
