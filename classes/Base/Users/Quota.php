 ...
 * Instead, change the Users_Quota.php file.
 *
 * @module Users
 */
/**
 * Base class representing 'Quota' rows in the 'Users' database
 * @class Base_Users_Quota
 * @extends Db_Row
 *
...
abstract class Base_Users_Quota extends Db_Row
{
	...
	{
		if (class_exists('Q_Config') and Q_Config::get('Db', 'connections', 'Users', 'indexes', 'Quota', false)) {
			return new Db_Expression(($with_db_name ? '{{dbname}}.' : '').'{{prefix}}'.'quota');
		} else {
			$conn = Db::getConnection('Users');
  			$prefix = empty($conn['prefix']) ? '' : $conn['prefix'];
  			$table_name = $prefix . 'quota';
  			if (!$with_db_name)
  				return $table_name;
  			$db = Db::connect('Users');
...
return array (
  ...
  3 => NULL,
	 ...
	 * @throws {Exception} An exception is thrown if $value is not string or is exceedingly long
	 */
	function beforeSet_resourceId($value)
	{
		if (!isset($value)) {
			$value='';
		}
		if ($value instanceof Db_Expression
               or $value instanceof Db_Range) {
			return array('resourceId', $value);
		}
		if (!is_string($value) and !is_numeric($value))
			throw new Exception('Must pass a string to '.$this->getTable().".resourceId");
		if (strlen($value) > 255)
			throw new Exception('Exceedingly long value being assigned to '.$this->getTable().".resourceId");
		return array('resourceId', $value);			
	}
	 ...
	 * @return {array} [[typeName, displayRange, modifiers, unsigned], isNull, key, default]
	 */
	static function column_resourceId()
	{
...
return array (
  ...
  3 => NULL,
	 ...
	 */
	function beforeSet_units($value)
	{
		if ($value instanceof Db_Expression
               or $value instanceof Db_Range) {
			return array('units', $value);
		}
		if (!is_numeric($value) or floor($value) != $value)
			throw new Exception('Non-integer value being assigned to '.$this->getTable().".units");
		$value = intval($value);
		if ($value < -2147483648 or $value > 2147483647) {
			$json = json_encode($value);
			throw new Exception("Out-of-range value $json being assigned to ".$this->getTable().".units");
		}
		return array('units', $value);			
	}

	/**
	 * @method maxSize_units
	 * Returns the maximum integer that can be assigned to the units field
	 * @return {integer}
	 */
	function maxSize_units()
	{
...
return array (
  ...
  3 => 'CURRENT_TIMESTAMP',
	...
	{
		$field_names = array('userId', 'resourceId', 'name', 'units', 'insertedTime');
		$result = $field_names;
