 ...
 * Instead, change the Users_Referred.php file.
 *
 * @module Users
 */
/**
 * Base class representing 'Referred' rows in the 'Users' database
 * @class Base_Users_Referred
 * @extends Db_Row
 *
...
abstract class Base_Users_Referred extends Db_Row
{
	...
	{
		if (class_exists('Q_Config') and Q_Config::get('Db', 'connections', 'Users', 'indexes', 'Referred', false)) {
			return new Db_Expression(($with_db_name ? '{{dbname}}.' : '').'{{prefix}}'.'referred');
		} else {
			$conn = Db::getConnection('Users');
  			$prefix = empty($conn['prefix']) ? '' : $conn['prefix'];
  			$table_name = $prefix . 'referred';
  			if (!$with_db_name)
  				return $table_name;
  			$db = Db::connect('Users');
	...
	static function insertManyAndExecute($rows = array(), $options = array())
	{
		self::db()->insertManyAndExecute(
			self::table(), $rows,
			array_merge($options, array('className' => 'Users_Referred'))
		);
	}
...
return array (
  ...
  3 => NULL,
	 ...
	 * @throws {Exception} An exception is thrown if $value is not string or is exceedingly long
	 */
	function beforeSet_toCommunityId($value)
	{
		if (!isset($value)) {
			$value='';
		}
	 ...
	 * @return {integer}
	 */
	function maxSize_toCommunityId()
	{
	 ...
	 * @return {array} [[typeName, displayRange, modifiers, unsigned], isNull, key, default]
	 */
	static function column_toCommunityId()
	{
...
return array (
  ...
  3 => NULL,
	 ...
	 * @throws {Exception} An exception is thrown if $value is not string or is exceedingly long
	 */
	function beforeSet_referredByUserId($value)
	{
		if (!isset($value)) {
			$value='';
		}
	 ...
	 * @return {integer}
	 */
	function maxSize_referredByUserId()
	{
	 ...
	 * @return {array} [[typeName, displayRange, modifiers, unsigned], isNull, key, default]
	 */
	static function column_referredByUserId()
	{
...
return array (
  ...
  3 => '{}',
	...
	function beforeSave($value)
	{
		if (!$this->retrieved) {
			$table = $this->getTable();
			foreach (array('userId','toCommunityId','referredByUserId') as $name) {
				if (!isset($value[$name])) {
		...
		if (!isset($this->fields["toCommunityId"]) and !isset($value["toCommunityId"])) {
			$this->toCommunityId = $value["toCommunityId"] = "";
		}
		if (!isset($this->fields["referredByUserId"]) and !isset($value["referredByUserId"])) {
			$this->referredByUserId = $value["referredByUserId"] = "";
		}
		return $value;			
	}
