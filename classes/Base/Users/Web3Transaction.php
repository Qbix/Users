 ...
 * Instead, change the Users_Web3Transaction.php file.
 *
 * @module Users
 */
/**
 * Base class representing 'Web3Transaction' rows in the 'Users' database
 * @class Base_Users_Web3Transaction
 * @extends Db_Row
 *
 ...
 */
abstract class Base_Users_Web3Transaction extends Db_Row
{
	/**
	 * @property $chainId
	 * @type string
	...
	static function table($with_db_name = true, $alias = null)
	{
		if (class_exists('Q_Config') and Q_Config::get('Db', 'connections', 'Users', 'indexes', 'Web3Transaction', false)) {
			...
			$conn = Db::getConnection('Users');
  			...
  			$table_name = $prefix . 'web3_transaction';
  			if (!$with_db_name)
  				return $table_name;
  			$db = Db::connect('Users');
...
return array (
  ...
  3 => NULL,
	 ...
	 * @throws {Exception} An exception is thrown if $value is not string or is exceedingly long
	 */
	function beforeSet_transactionId($value)
	{
		if (!isset($value)) {
			$value='';
		}
		if ($value instanceof Db_Expression
               or $value instanceof Db_Range) {
			return array('transactionId', $value);
		}
		if (!is_string($value) and !is_numeric($value))
			throw new Exception('Must pass a string to '.$this->getTable().".transactionId");
		if (strlen($value) > 100)
			throw new Exception('Exceedingly long value being assigned to '.$this->getTable().".transactionId");
		return array('transactionId', $value);			
	}
	 ...
	 * @return {integer}
	 */
	function maxSize_transactionId()
	{
	 ...
	 * @return {array} [[typeName, displayRange, modifiers, unsigned], isNull, key, default]
	 */
	static function column_transactionId()
	{
...
return array (
  ...
  3 => NULL,
	...
	function beforeSave($value)
	{
		if (!$this->retrieved) {
			$table = $this->getTable();
			foreach (array('chainId','transactionId') as $name) {
				if (!isset($value[$name])) {
